...
@Component({
  ...
  template: `
    <div class="fixed inset-0 bg-white bg-opacity-30 backdrop-blur-sm flex items-center justify-center z-50" 
         (click)="closeModal()">
      <div class="bg-white rounded-2xl w-[90%] max-w-[450px] p-8 relative shadow-[0_20px_60px_rgba(0,0,0,0.3)] border-2 border-gray-200 max-h-[90vh] overflow-y-auto" 
...
export class RegisterModalComponent {
  ...
  constructor(
    ...
    if (currentShop) {
      this.userData.shopId = currentShop._id;
    }
  }

  onSubmit(): void {
    if (!this.userData.fullName || !this.userData.email || !this.userData.phoneNumber || !this.userData.passwordHash) {
      this.errorMessage = 'Please fill in all fields';
      return;
    }
    
    if (this.userData.passwordHash.length < 6) {
      this.errorMessage = 'Password must be at least 6 characters';
      return;
    }
    
    if (!this.userData.shopId) {
      this.errorMessage = 'Shop information is missing';
      return;
    }
